...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function sendMessage() {
        ...
        await firebaseStorage.storeUserMessage(currentThreadId, message, messageId);
        ...
        try {
            ...
            await processAnswer(responseId, requestStartTime);
        } catch (error) {
    ...
    async function processAnswer(responseId, requestStartTime) {
        try {
            ...
            await firebaseStorage.storeAIResponse(currentThreadId, fullAnswer, `resp_${responseId}`, responseTime);
            
            // Store the full conversation
            await firebaseStorage.storeConversation(currentThreadId, conversationHistory);
...
`;
    ...
    // Handle window unload to store final conversation state
    window.addEventListener('beforeunload', function() {
        // Store the current conversation before the window unloads
        if (conversationHistory.length > 0) {
            firebaseStorage.storeConversation(currentThreadId, conversationHistory);
        }
    });
});